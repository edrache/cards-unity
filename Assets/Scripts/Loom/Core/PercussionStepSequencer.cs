...
{
    /// <summary>
    /// Expands one immutable percussion pattern into ordered events for sequential half-open windows.
    /// </summary>
    public sealed class PercussionStepSequencer
    {
        private readonly PercussionStepPattern pattern;
        ...
        /// </summary>
        public PercussionStepSequencer(PercussionStepPattern pattern, ulong seed, ulong streamId)
        {
        ...
        public bool TryScheduleThrough(
            ...
            SchedulingBuffer<ScheduledPercussionNoteEvent> destination)
        {
            if (destination == null)
            {
            ...
            while (TryGetCurrentEventStartTick(out long eventStartTick))
            {
                ...
                template.Step.TryGetNote(out PercussionNote note);
                var noteEvent = new PercussionNoteEvent(
                    note,
                    template.Step.Velocity,
                    eventStartTick,
                    template.Step.DurationTicks);
                var scheduledEvent = new ScheduledPercussionNoteEvent(
                    noteEvent,
                    nextSequenceNumber);
        ...
        private static TriggerTemplate[] BuildAndSortTemplates(PercussionStepPattern sourcePattern)
        {
            ...
            {
                PercussionPatternStep step = sourcePattern[stepIndex];
                if (step.IsRest)
                {
                    continue;
                }
        ...
        private static int CountTemplates(PercussionStepPattern sourcePattern)
        {
            long count = 0L;

            for (int index = 0; index < sourcePattern.Count; index++)
            {
                PercussionPatternStep step = sourcePattern[index];
                if (!step.IsRest)
                {
        ...
        private bool ShouldTrigger(PercussionPatternStep step, long sourceBaseTick)
        {
        ...
        private readonly struct TriggerTemplate
        {
            ...
            public PercussionPatternStep Step { get; }
