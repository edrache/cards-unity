...
namespace Loom.Core
{
    public readonly struct PercussionNoteEvent : IEquatable<PercussionNoteEvent>
    {
        public const int MinVelocity = 1;
        public const int MaxVelocity = 127;

        public PercussionNoteEvent(PercussionNote note, int velocity, long startTick, long durationTicks)
        {
            if (!note.IsValid)
            {
                throw new ArgumentException(
                    "Percussion note event requires a valid percussion note.",
                    nameof(note));
            }
        ...
        public PercussionNote Note { get; }
        ...
        public bool Equals(PercussionNoteEvent other)
        {
        ...
        {
            return obj is PercussionNoteEvent other && Equals(other);
        }
        ...
        {
            return $"Percussion slot {Note.SampleSlotId} at tick {StartTick} for "
                + $"{DurationTicks} ticks, velocity {Velocity}";
        }

        public static bool operator ==(PercussionNoteEvent left, PercussionNoteEvent right)
        {
