...
{
    /// <summary>
    /// Expands one immutable scale-degree pattern into harmony-resolved note events.
    /// </summary>
    public sealed class PitchedStepSequencer
    {
        private readonly PitchedStepPattern pattern;
        private readonly HarmonyResolver harmonyResolver;
        ...
        /// <summary>
        /// Precompiles every playable ratchet trigger and resolves its pitch at the final onset tick.
        /// </summary>
        public PitchedStepSequencer(
            PitchedStepPattern pattern,
            HarmonyResolver harmonyResolver,
            TrackRole role,
            ulong seed,
            ulong streamId)
        {
            this.pattern = pattern ?? throw new ArgumentNullException(nameof(pattern));
            this.harmonyResolver = harmonyResolver
                ?? throw new ArgumentNullException(nameof(harmonyResolver));
            ValidateRole(role);
            this.role = role;
            this.seed = seed;
            ...
            SchedulingBuffer<ScheduledNoteEvent> destination)
        {
            ...
            while (TryGetCurrentEventStartTick(out long eventStartTick))
            {
                ...
                template.Step.TryGetPitch(out ScaleDegreePitch pitch);
                Note note = harmonyResolver.Resolve(pitch, eventStartTick, role);
        ...
        private static void ValidateRole(TrackRole role)
        {
        ...
        private static TriggerTemplate[] BuildAndSortTemplates(PitchedStepPattern sourcePattern)
        {
            ...
            {
                PitchedPatternStep step = sourcePattern[stepIndex];
                if (step.IsRest)
                {
                    continue;
                }
        ...
        private static int CountTemplates(PitchedStepPattern sourcePattern)
        {
            long count = 0L;

            for (int index = 0; index < sourcePattern.Count; index++)
            {
                PitchedPatternStep step = sourcePattern[index];
                if (!step.IsRest)
                {
        ...
        private bool ShouldTrigger(PitchedPatternStep step, long sourceBaseTick)
        {
        ...
        private readonly struct TriggerTemplate
        {
            ...
            public PitchedPatternStep Step { get; }
