...
{
    public sealed class PercussionStepSequencerTests
    {
        [Test]
        public void ConstructorRejectsNullPattern()
        {
            Assert.Throws<ArgumentNullException>(
                () => new Core.PercussionStepSequencer(null, 0UL, 0UL));
        }
        ...
        public void SequencerEmitsPlayableStepsAndSkipsRests()
        {
            var pattern = new Core.PercussionStepPattern(
                ...
                new Core.PercussionPatternStep(new Core.PercussionNote(64), 90, 180L),
                Core.PercussionPatternStep.Rest);
            var sequencer = new Core.PercussionStepSequencer(pattern, 0UL, 0UL);
            var buffer = new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(4);

            Assert.That(sequencer.TryScheduleThrough(960L, buffer), Is.True);
            List<Core.ScheduledPercussionNoteEvent> events = Drain(buffer);
        ...
        {
            var sequencer = CreateQuarterPercussionStepSequencer(probability: 1f);
            var buffer = new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(4);

            Assert.That(sequencer.TryScheduleThrough(240L, buffer), Is.True);
            List<Core.ScheduledPercussionNoteEvent> firstWindow = Drain(buffer);
            ...
            List<Core.ScheduledPercussionNoteEvent> secondWindow = Drain(buffer);
        ...
        public void AllRestPatternAdvancesWatermarkWithoutEvents()
        {
            var pattern = new Core.PercussionStepPattern(
                ...
                Core.PercussionPatternStep.Rest);
            var sequencer = new Core.PercussionStepSequencer(pattern, 0UL, 0UL);
            var buffer = new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(1);
        ...
        {
            var zeroSequencer = CreateQuarterPercussionStepSequencer(probability: 0f);
            var oneSequencer = CreateQuarterPercussionStepSequencer(probability: 1f);
            var zeroBuffer = new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(8);
            var oneBuffer = new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(8);
        ...
        {
            var sequencer = CreateQuarterPercussionStepSequencer(probability: 0.5f);
            var buffer = new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(8);
        ...
        public void UnevenRatchetsUseFlooredOffsetsAndPreserveDuration()
        {
            ...
            {
                Assert.That(events[index].PercussionNoteEvent.DurationTicks, Is.EqualTo(120L));
            }
        }
        ...
        public void PositiveRatchetSpillAppearsInTheFollowingOutputCycle()
        {
            ...
            List<Core.ScheduledPercussionNoteEvent> beforeBoundary = Drain(buffer);
            Assert.That(GetStartTicks(beforeBoundary), Is.EqualTo(new[] { 239L }));

            Assert.That(sequencer.TryScheduleThrough(360L, buffer), Is.True);
            List<Core.ScheduledPercussionNoteEvent> atBoundary = Drain(buffer);
        ...
        public void AdjacentIrregularWindowsMatchOneLargeWindow()
        {
            ...
            var oneShotSequencer = CreateQuarterPercussionStepSequencer(probability: 0.5f);
            var partitionedSequencer = CreateQuarterPercussionStepSequencer(probability: 0.5f);
            var oneShotBuffer = new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(8);
            var partitionedBuffer = new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(8);

            Assert.That(
                oneShotSequencer.TryScheduleThrough(1_920L, oneShotBuffer),
                Is.True);
            List<Core.ScheduledPercussionNoteEvent> oneShotEvents = Drain(oneShotBuffer);
            var partitionedEvents = new List<Core.ScheduledPercussionNoteEvent>();
        ...
        public void CapacityOneBackpressureResumesWithoutLossOrDuplication()
        {
            ...
            var sequencer = new Core.PercussionStepSequencer(pattern, 0UL, 0UL);
            var buffer = new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(1);
            var events = new List<Core.ScheduledPercussionNoteEvent>();
        ...
        {
            var sequencer = CreateQuarterPercussionStepSequencer(probability: 0f);
            var buffer = new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(1);
            sequencer.TryScheduleThrough(960L, buffer);
        ...
        public void SchedulingRejectsInvalidArguments()
        {
            ...
            Assert.Throws<ArgumentOutOfRangeException>(
                () => sequencer.TryScheduleThrough(
                    -1L,
                    new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(1)));
        }
        ...
        public void ResetReplaysTheSameDeterministicStream()
        {
            ...
            List<Core.ScheduledPercussionNoteEvent> first = Drain(buffer);
            sequencer.Reset();
            sequencer.TryScheduleThrough(1_920L, buffer);
            List<Core.ScheduledPercussionNoteEvent> replay = Drain(buffer);
        ...
        {
            var sequencer = CreateQuarterPercussionStepSequencer(probability: 1f);
            var buffer = new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(2);

            sequencer.Reset(240L);
            Assert.That(sequencer.TryScheduleThrough(241L, buffer), Is.True);

            Assert.That(buffer.TryDequeue(out Core.ScheduledPercussionNoteEvent scheduledEvent), Is.True);
        ...
        {
            var negativePattern = new Core.PercussionStepPattern(
                4,
                new Core.PercussionPatternStep(
                    new Core.PercussionNote(60),
                    100,
                    120L,
                    microtimingTickOffset: -10));
            var negativeSequencer = new Core.PercussionStepSequencer(negativePattern, 0UL, 0UL);
            var negativeBuffer =
                new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(2);
            negativeSequencer.Reset(230L);

            Assert.That(negativeSequencer.TryScheduleThrough(231L, negativeBuffer), Is.True);
            Assert.That(
                negativeBuffer.TryDequeue(out Core.ScheduledPercussionNoteEvent negativeEvent),
                Is.True);
            Assert.That(negativeEvent.StartTick, Is.EqualTo(230L));

            var positivePattern = new Core.PercussionStepPattern(
                4,
                new Core.PercussionPatternStep(
                    new Core.PercussionNote(60),
                    100,
                    ...
                    ratchetCount: 2,
                    microtimingTickOffset: 239));
            var positiveSequencer = new Core.PercussionStepSequencer(positivePattern, 0UL, 0UL);
            var positiveBuffer =
                new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(2);
            ...
            Assert.That(
                positiveBuffer.TryDequeue(out Core.ScheduledPercussionNoteEvent positiveEvent),
                Is.True);
        ...
        public void EventEndOverflowFailsWithoutCompletingTheWindow()
        {
            var pattern = new Core.PercussionStepPattern(
                4,
                new Core.PercussionPatternStep(new Core.PercussionNote(60), 100, long.MaxValue));
            var sequencer = new Core.PercussionStepSequencer(pattern, 0UL, 0UL);
            var buffer = new Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent>(2);
        ...
        public void SchedulingHotPathAllocatesNoManagedMemoryAfterInitialization()
        {
            ...
            for (int iteration = 0; iteration < 10_000; iteration++)
            {
                ...
                allOperationsSucceeded &= buffer.TryDequeue(
                    out Core.ScheduledPercussionNoteEvent scheduledEvent);
                checksum += (long)scheduledEvent.PercussionNoteEvent.Note.SampleSlotId;
                sequencer.Reset();
            }
        ...
        private static Core.PercussionStepSequencer CreateQuarterPercussionStepSequencer(float probability)
        {
            var pattern = new Core.PercussionStepPattern(
                4,
                new Core.PercussionPatternStep(
                    ...
                    120L,
                    probability));
            return new Core.PercussionStepSequencer(pattern, 0UL, 0UL);
        }

        private static List<Core.ScheduledPercussionNoteEvent> Drain(
            Core.SchedulingBuffer<Core.ScheduledPercussionNoteEvent> buffer)
        {
            var events = new List<Core.ScheduledPercussionNoteEvent>(buffer.Count);
            while (buffer.TryDequeue(out Core.ScheduledPercussionNoteEvent scheduledEvent))
            {
        ...
        private static long[] GetStartTicks(List<Core.ScheduledPercussionNoteEvent> events)
        {
        ...
        private static void AssertEvent(
            Core.ScheduledPercussionNoteEvent scheduledEvent,
            ulong sampleSlotId,
            int velocity,
            long startTick,
            long durationTicks,
        ...
        {
            Assert.That(scheduledEvent.PercussionNoteEvent.Note.SampleSlotId, Is.EqualTo(sampleSlotId));
            Assert.That(scheduledEvent.PercussionNoteEvent.Velocity, Is.EqualTo(velocity));
            Assert.That(scheduledEvent.StartTick, Is.EqualTo(startTick));
            Assert.That(scheduledEvent.PercussionNoteEvent.DurationTicks, Is.EqualTo(durationTicks));
