...
- `TorchInteraction` on `Shadow Knight` provides the pickup-only `TorchInteract` Rewired action (id 5, keyboard E) and a Polish prompt naming the available item. The Input System fallback also uses E. Gamepad mapping can be assigned in the Rewired inspector. With empty hands, the character selects the nearest unheld, active torch within Pickup Range and clear reach, including corpse torches. A held torch cannot be replaced using E; it must first be lost through a gameplay event. Treasure uses the session inventory described below.
- Action-driven dropping has been removed; knockdown still drops the torch immediately. Picking up turns toward the torch, crouches using the gait's existing leg IK, and reaches for the grip using arm IK. A short recovery blends back to the holding pose. Movement pauses during the interaction; attacks and repeated interaction presses are rejected until it finishes. Attacks require a held torch.
...
- Direct Play Mode checks in the saved 15-room cave: 0/2/8 coins per room yielded 0/30/120 coins, always worth 1, alongside eight unchanged large treasures. Rebuilding reproduced coin positions. Mesh vertices retained at least 0.008 m clearance over the analytic floor; no overlaps with rock colliders were found. A close-up camera capture verified the yellow coin under a standalone torch and the dither pipeline. No console errors; temporary objects discarded and Edit Mode restored. These are direct editor checks, not a persistent automated suite; collection is now handled by TorchInteraction and CharacterInventory.


## Treasure collection and inventory

- E only picks up items and never drops a held torch. The nearest reachable Treasure takes priority; with a held torch the left arm reaches while the right keeps holding it, and without one the right arm collects treasure or a ground torch. Only events such as CharacterKnockdown call HandheldTorch.Drop. The bottom-screen prompt names the selected item (for example, "[E] Podnieś: Złota moneta" or "[E] Podnieś: pochodnię") and disappears when no pickup is available.
- TorchInteraction resolves the chosen rig arm/forearm/hand on each interaction, uses the existing crouch and two-bone reach, and rechecks range, activity and obstruction at transfer. Cancellation before contact leaves the treasure in place. Movement and attack restrictions remain unchanged.
- CharacterInventory is added beside TorchInteraction at runtime if absent. Collected treasures immediately deactivate and are destroyed; entries group by display name and unit value. The simple Polish screen panel shows quantities and total value with a scrollable list. Inventory lasts for this character instance/session; no save system is added. All existing Treasure prefabs, including coins, use this flow.
- Direct Play Mode method checks verified left-hand collection while retaining the torch, right-hand empty collection, ground torch pickup, repeated coin stacking, chalice value, cancellation and wall rejection. Four coins plus a chalice yielded five items, two groups and value 104. Near-contact hand errors were under 0.2 mm on the saved rig. A camera capture of left-hand reach was inspected; console errors were absent. Temporary objects were discarded and Edit Mode restored. These were direct methods, not physical E-key tests or an automated suite; the GUI was not visually captured, and extreme terrain/proportion combinations were not checked.

- Pickup-only follow-up checks: direct Play Mode methods rejected E with a held torch and no treasure, kept the torch through coin collection, and confirmed CharacterKnockdown still drops it. ActionLabel returned the coin name, the ground torch name, and an empty string with no available target. Console contained no errors; Edit Mode restored. Physical E input and a visual GUI capture were not tested.
