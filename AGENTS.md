...
- `ProceduralCave` exposes `Treasure Prefab` and `Treasure Room Percentage` (default 50). A separate seeded shuffle populates selected rooms with values 50–150 in tens. Generated treasures follow floor slopes at reserved positions beside room centres; rock placement reserves their footprint. Rebuilding replaces them and reproduces positions/values. The saved 15-room scene generates eight chalices.
