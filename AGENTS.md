...
- E prioritizes the nearest reachable Treasure over dropping a held torch. With a held torch the left arm reaches while the right keeps holding it; without one the right arm collects either treasure or a ground torch. If no treasure is nearby, the existing contextual torch drop/pickup remains available.
