...
- `TorchInteraction` on `Shadow Knight` provides the contextual `TorchInteract` Rewired action (id 5, keyboard E) and a small Polish "Upuść" / "Podnieś" prompt. The Input System fallback also uses E. Gamepad mapping can be assigned in the Rewired inspector. The character remembers its own dropped torch; this does not implement an inventory or swapping between unrelated torches.
...
- `Pickup Range` (default 1 m), `Drop Duration`, `Pickup Duration`, `Recovery Duration` and `Show Prompt` live on `TorchInteraction`. Pickup requires a settled torch, a clear line to its grip and a reachable hand target at the moment of attachment. Burned-out torches can still be picked up.
...
- These are visual styles, with no exhaustion meter, movement-speed restriction, shortened CharacterController, knockdown or get-up state yet. Future knockdown/get-up should blend persistent posture separately from distance-driven locomotion and coordinate with torch pose suppression.
